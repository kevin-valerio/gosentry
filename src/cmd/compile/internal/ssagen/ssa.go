...
func (s *state) checkWithMessage(cmp *ssa.Value, fn *obj.LSym, msg string) {
	...
	if bPanic == nil {
		...
		msgVal := s.entryNewValue0A(ssaop.OpConstString, types.Types[types.TSTRING], ssa.StringToAux(msg))
		// The panic call takes/returns memory to ensure that the right
		// memory state is observed if the panic happens.
...
func (s *state) checkTypeTruncation(n ir.Node, value *ssa.Value, fromType, toType *types.Type, op ssaop.Op) *ssa.Value {
	...
	maxConst := s.newValue0I(ssaop.OpConst64, types.Types[types.TINT64], maxVal)
	minConst := s.newValue0I(ssaop.OpConst64, types.Types[types.TINT64], minVal)
	...
	if fromType.Size() < 8 {
		if fromType.IsSigned() {
			...
			case 1:
				compareValue = s.newValue1(ssaop.OpSignExt8to64, types.Types[types.TINT64], value)
			case 2:
				compareValue = s.newValue1(ssaop.OpSignExt16to64, types.Types[types.TINT64], value)
			case 4:
				compareValue = s.newValue1(ssaop.OpSignExt32to64, types.Types[types.TINT64], value)
			}
		} else {
			// Zero extend smaller unsigned types to int64
			switch fromType.Size() {
			case 1:
				compareValue = s.newValue1(ssaop.OpZeroExt8to64, types.Types[types.TINT64], value)
			case 2:
				compareValue = s.newValue1(ssaop.OpZeroExt16to64, types.Types[types.TINT64], value)
			case 4:
				compareValue = s.newValue1(ssaop.OpZeroExt32to64, types.Types[types.TINT64], value)
			}
		}
	}

	// Check if value is within bounds: minVal <= value <= maxVal
	geMin := s.newValue2(ssaop.OpLeq64, types.Types[types.TBOOL], minConst, compareValue)
	leMax := s.newValue2(ssaop.OpLeq64, types.Types[types.TBOOL], compareValue, maxConst)
	inBounds := s.newValue2(ssaop.OpAndB, types.Types[types.TBOOL], geMin, leMax)
...
func (s *state) intAdd(n ir.Node, a, b *ssa.Value) *ssa.Value {
	...
	if n.Type().IsSigned() {
		...
		aGeZero := s.newValue1(ssaop.OpNot, types.Types[types.TBOOL], aLtZeroTemp)
		// Check if b >= 0 (use !(b < 0))
		bLtZeroTemp := s.newValue2(s.ssaOp(ir.OLT, b.Type), types.Types[types.TBOOL], b, zero)
		bGeZero := s.newValue1(ssaop.OpNot, types.Types[types.TBOOL], bLtZeroTemp)
		// Check if result < a
		...
		bothPos := s.newValue2(ssaop.OpAndB, types.Types[types.TBOOL], aGeZero, bGeZero)
		posOverflow := s.newValue2(ssaop.OpAndB, types.Types[types.TBOOL], bothPos, resultLtA)
		...
		bothNeg := s.newValue2(ssaop.OpAndB, types.Types[types.TBOOL], aNeg, bNeg)
		negOverflow := s.newValue2(ssaop.OpAndB, types.Types[types.TBOOL], bothNeg, resultGtA)

		// Overall overflow condition
		overflow := s.newValue2(ssaop.OpOrB, types.Types[types.TBOOL], posOverflow, negOverflow)

		// s.checkWithMessage() panics when condition is FALSE, so pass "no overflow" condition
		noOverflow := s.newValue1(ssaop.OpNot, types.Types[types.TBOOL], overflow)
		...
		// s.checkWithMessage() panics when condition is FALSE, so pass "no overflow" condition
		noOverflow := s.newValue1(ssaop.OpNot, types.Types[types.TBOOL], resultLtA)
		errorMsg := formatOverflowMessage(n.Op(), n.Type())
		s.checkWithMessage(noOverflow, ir.Syms.Panicoverflowdetailed, errorMsg)
	}
...
func (s *state) intSub(n ir.Node, a, b *ssa.Value) *ssa.Value {
	...
	if n.Type().IsSigned() {
		...
		aGeZero := s.newValue1(ssaop.OpNot, types.Types[types.TBOOL], aLtZeroTemp2)
		// Check if a < 0
		...
		posOverflow := s.newValue2(ssaop.OpAndB, types.Types[types.TBOOL],
			s.newValue2(ssaop.OpAndB, types.Types[types.TBOOL], aGeZero, bLtZero), resultLtA)

		// Negative overflow: a < 0 && b > 0 && result > a
		negOverflow := s.newValue2(ssaop.OpAndB, types.Types[types.TBOOL],
			s.newValue2(ssaop.OpAndB, types.Types[types.TBOOL], aLtZero, bGtZero), resultGtA)
		...
		noUnderflow := s.newValue1(ssaop.OpNot, types.Types[types.TBOOL], aLtB)
		errorMsg := formatOverflowMessage(n.Op(), n.Type())
...
func (s *state) intMul(n ir.Node, a, b *ssa.Value) *ssa.Value {
	...
	eitherZero := s.newValue2(ssaop.OpOrB, types.Types[types.TBOOL], aIsZero, bIsZero)
	...
	validMul := s.newValue2(ssaop.OpOrB, types.Types[types.TBOOL], eitherZero, quotientOk)
...
func (s *state) intDiv(n ir.Node, a, b *ssa.Value) *ssa.Value {
	...
	overflow := s.newValue2(ssaop.OpAndB, types.Types[types.TBOOL], aIsMinInt, bIsNegOne)
		...
		debugInfo.GetPC = func(b, v ssa.ID) int64 {
			...
			case ssa.BlockStart.ID:
				if b == f.Entry.ID {
					...
					// this should only happen for function args (ssaop.OpArg)
				}
				return bstart[b].Pc
