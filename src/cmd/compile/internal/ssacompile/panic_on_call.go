...
func panicOnCall(f *ssa.Func) {
	...
	for _, b := range f.Blocks {
		for i := 0; i < len(b.Values); i++ {
			v := b.Values[i]
			if v.Op != ssaop.OpStaticCall && v.Op != ssaop.OpStaticLECall {
				continue
			}
			aux, ok := v.Aux.(*ssa.AuxCall)
			...
			_, sb := f.SpSb()
			dataSym := f.Frontend().StringData(target)
			ptr := b.NewValue1A(v.Pos, ssaop.OpAddr, f.Config.Types.BytePtr, ssa.SymToAux(dataSym), sb)
			lenVal := b.NewValue0I(v.Pos, ssaop.OpConst64, f.Config.Types.Uintptr, int64(len(target)))

			auxCall := ssa.StaticAuxCall(ir.Syms.PanicOnCall, f.ABIDefault.ABIAnalyzeTypes([]*types.Type{
				f.Config.Types.BytePtr,      // data pointer
				types.Types[types.TUINTPTR], // length
			}, nil))
			panicCall := b.NewValue0A(v.Pos, ssaop.OpStaticCall, types.TypeMem, auxCall)
			...
			vals := b.Values
			newVals := make([]*ssa.Value, 0, len(vals))
